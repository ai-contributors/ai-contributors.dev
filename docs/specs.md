...
**Status:** Draft v0.1
**Target site:** `https://ai-contributors.dev/ai-contributor-spec/` (custom domain on GitHub Pages)
...
- **Start here** — `/ai-contributor-spec/`, `/ai-contributor-spec/specification/`, `/ai-contributor-spec/levels/`
...
7. **Pillars grid** — 7 cards, one per pillar (Engineering Foundation 🏗️, Security 🛡️, Quality & Reliability 🎯, Release 🚀, AI Agents 🤖, AI Risk ⚠️, Oversight 🧭), each with the clause range and a link to the relevant `/ai-contributor-spec/specification/#…` anchor.
...
- **10.3. Deploy workflow** — `.github/workflows/deploy.yml` **must**:
  ...
  - run `pnpm build`,
  - build production with Astro base `/ai-contributor-spec/`,
  - on `main` only: copy `dist/` into `gh-pages/ai-contributor-spec/`, write a static root redirect to `/ai-contributor-spec/`, and push the generated `gh-pages` branch.
...
- [ ] Bumping the submodule pointer to a newer spec version tag on `main` updates `/ai-contributor-spec/specification/` within 5 minutes of the merge.
...
- [ ] `/ai-contributor-spec/llms.txt` and `/ai-contributor-spec/llms-full.txt` exist and validate against the [`llmstxt.org`](https://llmstxt.org) format.
- [ ] `/ai-contributor-spec/llms-full.txt` includes the specification, audit model, audit prompt, adoption guide, coverage matrix, changelog, and skill documentation.
